...
You must select only one speaker to go next (i.e., from the set {[agent.name for agent in agents]}). Provide their name, PRECCEDED by a brief explanation for your choice. Do not mention any other participant by name.
"""
    ...
    def select_speaker_prompt(self, agents: List[Agent]):
        ...
        return f"Read the above conversation, then carefully consider who should speak next based on who's input would be most valued in this moment to make progress on the task. Select the next speaker from {[agent.name for agent in agents]}. Return their name PRECCEDED by a brief explanation for your choice. Do not mention any other participant by name."
